...
public class DepthData {
    ...
    private static FloatBuffer convertRawDepthImagesTo3dPointBuffer(
        ...
        for (int y = 0; y < depthHeight; y += step) {
            for (int x = 0; x < depthWidth; x += step) {
                ...
                Log.d(TAG, "convertRawDepthImagesTo3dPointBuffer: "+ Arrays.toString(data));
            }
        }
    ...
    private static void sendRGB(Image cameraImage){
        /*final Image.Plane depthImagePlane = cameraImage.getPlanes()[0];
        ByteBuffer depthByteBufferOriginal = depthImagePlane.getBuffer();
        ByteBuffer depthByteBuffer = ByteBuffer.allocate(depthByteBufferOriginal.capacity());
        depthByteBuffer.order(ByteOrder.LITTLE_ENDIAN);
        while (depthByteBufferOriginal.hasRemaining()) {
        ...
        depthByteBuffer.rewind();
        ShortBuffer depthBuffer = depthByteBuffer.asShortBuffer();
        Log.d(TAG, "sendRGB: "+depthBuffer.get(45));*/

        Image.Plane[] planes = cameraImage.getPlanes();
        int yRowStride = planes[0].getRowStride();
        byte[] yImage = new byte[yRowStride];
        ByteBuffer k = planes[0].getBuffer().get(yImage);

        Log.d(TAG, "sendRGB: "+Arrays.toString(k.array()));
